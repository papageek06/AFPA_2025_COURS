...
if(isset($_GET["state"]) && $_GET["state"] != "none") {
    $state = $_GET["state"];
    $stmt = $pdo->query("SELECT * FROM orders WHERE state = '$state' ORDER BY created_at DESC");
} else if(isset($_GET["id_order"]) && $_GET["id_order"] != "") {
    $id = $_GET["id_order"];
    $stmt = $pdo->query("SELECT * FROM orders WHERE id = '$id' ORDER BY created_at DESC");
} else {
